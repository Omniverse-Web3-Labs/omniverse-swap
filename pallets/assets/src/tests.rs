...
use pallet_omniverse_protocol::{traits::OmniverseAccounts, OmniverseTokenProtocol, TokenOpcode, TransferTokenOp, MintTokenOp, TRANSFER, MINT};
...
fn force_creating_should_fail() {
	new_test_ext().execute_with(|| {
		assert_noop!(Assets::force_create(RuntimeOrigin::root(), 0, 1, false, 1), Error::<Test>::Unsupport);
	});
}
...
fn destroying_should_fail() {
	new_test_ext().execute_with(|| {
		assert_noop!(Assets::destroy(RuntimeOrigin::signed(1), 0, DestroyWitness { accounts: 0, sufficients: 0, approvals: 0 }), Error::<Test>::Unsupport);
	});
}
...
fn transferring_should_fail() {
	new_test_ext().execute_with(|| {
		assert_noop!(Assets::transfer(RuntimeOrigin::signed(1), 0, 1, 91), Error::<Test>::Unsupport);
	});
}
...
fn transferring_keep_alive_should_fail() {
	new_test_ext().execute_with(|| {
		assert_noop!(Assets::transfer_keep_alive(RuntimeOrigin::signed(1), 0, 1, 91), Error::<Test>::Unsupport);
	});
}
...
fn force_transferring_should_fail() {
	new_test_ext().execute_with(|| {
		assert_noop!(Assets::force_transfer(RuntimeOrigin::signed(1), 0, 1, 2, 91), Error::<Test>::Unsupport);
	});
}
...
fn transferring_ownership_should_fail() {
	new_test_ext().execute_with(|| {
		assert_noop!(Assets::transfer_ownership(RuntimeOrigin::signed(1), 0, 1), Error::<Test>::Unsupport);
	});
}
...
fn setting_team_should_fail() {
	new_test_ext().execute_with(|| {
		assert_noop!(Assets::set_team(RuntimeOrigin::signed(1), 0, 1, 2, 91), Error::<Test>::Unsupport);
	});
}
...
fn force_asset_status_should_fail() {
	new_test_ext().execute_with(|| {
		assert_noop!(Assets::force_asset_status(RuntimeOrigin::signed(1), 0, 1, 2, 2, 2, 10, false, false), Error::<Test>::Unsupport);
	});
}
...
fn approving_transfer_should_fail() {
	new_test_ext().execute_with(|| {
		assert_noop!(Assets::approve_transfer(RuntimeOrigin::signed(1), 0, 1, 91), Error::<Test>::Unsupport);
	});
}
...
fn canceling_approval_should_fail() {
	new_test_ext().execute_with(|| {
		assert_noop!(Assets::cancel_approval(RuntimeOrigin::signed(1), 0, 1), Error::<Test>::Unsupport);
	});
}
...
fn force_setting_team_should_fail() {
	new_test_ext().execute_with(|| {
		assert_noop!(Assets::force_cancel_approval(RuntimeOrigin::signed(1), 0, 1, 2), Error::<Test>::Unsupport);
	});
}
...
fn transferring_approved_should_fail() {
	new_test_ext().execute_with(|| {
		assert_noop!(Assets::transfer_approved(RuntimeOrigin::signed(1), 0, 1, 2, 91), Error::<Test>::Unsupport);
	});
}
...
fn it_fails_for_create_token_with_token_already_exist() {
    new_test_ext().execute_with(|| {
        ...
        assert_err!(Assets::create_token(RuntimeOrigin::signed(1), pk, vec![1], None), Error::<Test>::InUse);
    });
}
